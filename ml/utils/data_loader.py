...
def load_categorizer_data():
    ...
    dataset = load_dataset(
        dataset_name,
        token=backend_settings.HF_TOKEN,
    )
...
def load_forecaster_data():
    ...
    if not csv_files:
        logger.error(
            "Expenses_clean.csv not found inside %s",
            path,
        )
        return pd.DataFrame()
    ...
    logger.info(
        "Successfully loaded %d expense records.",
        len(forecast_df),
    )
